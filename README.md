...
> **Also in this repo: [LUMITAL](lumital/README.md)** — a second game: a psychedelic
> multiverse evolution explorer (108 creature species, black-hole lensing,
> fractal mazes, six worlds). `npm run dev:lumital` → port **5174**.
...
| On foot (combat) | `Q` punch · `F` shoot · `T` throw held item · `E` pick up items |
...
| Main menu | `ui/menu/MainMenu.tsx`, settingsStore `menuOpen` | boot / re-open | AAA overlay over the live scene; cinematic orbit camera (CameraRig menu branch) |
| Districts (v1.1) | `lib/geometry/biomegen.ts`, `environment/BiomeField.tsx`, `environment/biomeCollision.ts` | seeded per world | walk-in villages (rapier wall colliders, furnished), farms (instanced crops, barns, silos, windmills), reflective lake (drei MeshReflectorMaterial) |
| Items & combat (v1.1) | `lib/simulation/items.ts`, `combat.ts`, `inventoryStore`, `entities/Pickups.tsx`, `entities/Combat.tsx` | 10 Hz pickup / event verbs | seeded item orbs, 5-slot hotbar, punch (cone melee), shoot (hitscan tracer), throw (dynamic rapier balls); NPCs flee when hit |
