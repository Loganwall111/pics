...
export function NPCManager(): React.JSX.Element {
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < NPC_ROSTER.length; i++) {
      ...
      const panicking = t < (npcShared.panicUntil[i] ?? -1);
      if (panicking && !engaged) {
        // Flee: run directly away from the player, faster than the stroll.
        const dx = w.x - pp.x;
        const dz = w.z - pp.z;
        const len = Math.hypot(dx, dz) || 1;
        w.x += (dx / len) * 4.2 * delta;
        w.z += (dz / len) * 4.2 * delta;
        w.phase += 4.2 * delta * 2.6;
        w.yaw = dampAngle(w.yaw, Math.atan2(dx, dz), 10, delta);
        group.position.set(w.x, Math.abs(Math.sin(t * 6 + i)) * 0.09, w.z);
        group.rotation.y = dampAngle(group.rotation.y, w.yaw, 12, delta);
      } else if (!engaged) {
...
export const npcShared = {
  ...
  /** Panic deadlines (shader-clock seconds) per roster index — written by
   * the combat system, read here: panicked NPCs flee the player. */
  panicUntil: NPC_ROSTER.map(() => -1),
};
