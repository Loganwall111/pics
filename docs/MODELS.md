...
## Districts, items, combat, main menu (v1.1)

No new art assets — everything procedural.

- **Villages** (`biomegen.ts`): 6 houses each at 420 u (N + E clusters), walls
  are rapier fixed cuboids with a 1.5 m door gap — you physically walk in.
  Furnished (table/bed/shelf/warm lamp light). Well with rotating roof.
- **Farms**: 8 plots at ~600 u, 3 crop types as instanced rows (~300/plant),
  barns walk-in, silos, 22-post fence rings, windmills with turning blades.
- **Lake** at (−80, 640), r = 64: drei `MeshReflectorMaterial` — true planar
  screen-space reflection (1024 px, blurred), metalness-driven.
- **Combat math** (`combat.ts`, tested): cone target selection (range +
  half-angle + yaw wrap), ballistic throw velocity (45° max range pinned),
  cooldown gate. Punched NPCs panic-flee 4 s at 4.2 m/s (`npcShared.panicUntil`).
- **Items** (`items.ts`, tested): 5 stackable ids (×8), 5 slots, seeded
  scatter (plaza ring + villages + interiors), 20 s respawn.
- **Main menu**: `menuOpen` gates CameraRig into a cinematic orbit (r = 74,
  y = 26±6, lookAt plaza); ENTER exits to gameplay seamlessly.
